...
// wp fix 
if (_r isEqualTo 0) then {_r = 1000;};

// fsm level fixes (clean in 2.0)
[leader _grp,99,999999] call lambs_danger_fnc_leaderModeUpdate;
_grp setVariable ["dangerAI","disabled"];
...
while {{alive _x} count units _grp > 0} do {
  ...
  if (!isNull _t) then  {
    ...
    // DEBUG
    if (lambs_danger_debug_functions) then {systemchat format ["danger.wp taskHunt: %1 targets %2 at %3 Meters",_grp,name _t,floor (leader _grp distance _t)]};
