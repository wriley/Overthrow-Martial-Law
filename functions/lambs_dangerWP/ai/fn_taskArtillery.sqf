...
sleep (linearConversion [50,1500,(_gun distance _pos),30,60,true]);
...
if (canFire _gun && {alive _caller}) then {
	...
	for "_check" from 1 to (1 + random 2) do {
		...
		for "_i" from 1 to (1 + random 1) do {
			...
			if (lambs_danger_debug_functions) then {
				_m = [_target,format ["%1 (Check round %2)",getText (configFile >> "CfgVehicles" >> (typeOf _gun) >> "displayName"),_i],"Color4_FD_F","hd_destroy"] call lambs_danger_fnc_dotMarker;
				_mlist pushBack _m;
			}; 
	...
	for "_i" from 1 to _rounds do {
		...
		// debug 
		if (lambs_danger_debug_functions) then {
			_m = [_target,format ["%1 (Round %2)",getText (configFile >> "CfgVehicles" >> (typeOf _gun) >> "displayName"),_i],"colorIndependent","hd_destroy"] call lambs_danger_fnc_dotMarker;
			_mlist pushBack _m;
		}; 
