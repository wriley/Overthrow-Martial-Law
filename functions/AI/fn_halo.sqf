/*
	...
	2: (optional) AUTOOPEN -(number) auto open parachute at 150m by default
	3: (optional) SAVELOADOUT - (boolean) true to save backpack and its contents, and add a "fake" backpack to the front of the unit.
	4: (optional) CHEMLIGHT - (boolean) true if the units will use chemlights
...
_autoOpen 	 = [_this, 2, 150, [150]] call BIS_fnc_param;
_saveLoadOut = [_this, 3, true, [true]] call BIS_fnc_param;
_chemLight	 = [_this, 4, false, [false]] call BIS_fnc_param;

//Validate parameters
if (isNull _unit) exitWith {"Unit parameter must not be objNull. Accepted: OBJECT" call BIS_fnc_error};
if (_altitude < 250) exitWith {"Altitude is too low for HALO. Accepted: 250 and greater." call BIS_fnc_error};
...
if (isPlayer _unit) then {
	[_unit,_autoOpen] spawn {
		...
		waitUntil {(getPos _unit select 2) < _autoOpen || animationState _unit == "para_pilot" && alive _unit};
