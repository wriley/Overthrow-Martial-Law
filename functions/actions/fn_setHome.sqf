closeDialog 0;
diag_log format ["setHome params:%1", _this];
...
if(typename _b isEqualTo "ARRAY") then {
	...
	private _cost = _price * (_damage / 100);
	if (_cls in OT_allRepairableRuins) exitWith {
		private _ruins = [];
		{
			_x params ["_ruinClass"];
			if(_cls isEqualTo _ruinClass) exitWith {_ruins = _x};
	...
	if (_cls in OT_allRepairableBuildings) exitWith {
		...
		if(_money >= _cost) then {
			...
			format["You need $%1 to repair this building",[_cost, 1, 0, true] call CBA_fnc_formatNumber];
		};
		_handled = true;
	};
	...
	if(!isNil "_owner") then {
		if (_cls in OT_allBuyableBuildings && _owner isEqualTo getplayerUID player) exitWith {
			_handled = true;
