...
if(!(typeOf _dest in [OT_warehouse]) && ( _supplycache isEqualType "")) then {
...
player globalchat format ["transferHelper: _source:%1 _dest:%2", _source, _dest];
if (typeOf _dest in [OT_warehouse]) then {
	_toname = "Warehouse";
	_iswarehouse = true;
	_warehouse = _dest call OT_fnc_nearestWarehouse select 1;
} else {
...
if (_iswarehouse) then {
	player globalchat "calling addToWarehouse";
	{
		...
		[_warehouse, _cls, _qtyIn] remoteExecCall ["OT_fnc_addToWarehouse", 2, false];
