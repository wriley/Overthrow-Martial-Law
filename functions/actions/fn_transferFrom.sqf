...
private _objects = [];
{
	if (!(_x isEqualTo _veh) && {(_x isKindOf "Car" or _x isKindOf "Air" or _x isKindOf "ReammoBox_F" or _x isKindOf "Ship") && !(side _x isEqualTo west) && !({alive _x} count crew _x > 0)}) then {_objects pushback _x};
