...
vehicle _unit suppressFor (4 + random 4);

// debug
if (lambs_danger_debug_functions) then {systemchat format ["%1 suppression (%2 @ %3m)",side _unit,getText (configFile >> "CfgVehicles" >> (typeOf vehicle _unit) >> "displayName"),round (_unit distance _pos)];};
