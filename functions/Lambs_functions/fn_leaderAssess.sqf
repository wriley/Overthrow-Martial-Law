...
[_unit,99,66] call lambs_danger_fnc_leaderModeUpdate;
...
if (count _enemy > 0) then {
    ...
    if (count _targets > 0) then {
        [_unit,3,(_unit getHideFrom (_targets select 0))] call lambs_danger_fnc_leaderMode;
    };

    // Enemy is Tank/Air?
    _targets = _enemy select {_x isKindOf "Air" || {_x isKindOf "Tank" && {_x distance2d _unit < 400}}};
    if (count _targets > 0) then {
        [_unit,2,_targets select 0] call lambs_danger_fnc_leaderMode;
    };
    ...
    if (count _targets > 0 && {count (missionNameSpace getVariable ["lambs_artillery_" + str (side _unit),[]]) > 0}) then {
        [_unit,6,(_unit getHideFrom (_targets select 0))] call lambs_danger_fnc_leaderMode;
    };

    // communicate <-- possible remove?
    [_unit,selectRandom _enemy] call lambs_danger_fnc_shareInformation;

};

// Check nearby houses?
if (random 1 > 0.4 && {_unit distance (nearestBuilding _pos) < 80}) then {
        [_unit,4,_pos] call lambs_danger_fnc_leaderMode;
};
