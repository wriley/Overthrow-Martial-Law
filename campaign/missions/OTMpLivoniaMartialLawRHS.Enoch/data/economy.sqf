OT_economicData = [
    [[11556.2,7074.01,0],"Power Plant"],
    [[1662.92,3688.21,0],"Polana Sawmill","","OT_Wood"],
    [[2558.82,5523.93,0],"Topolin Cattle Yards","OT_Wood","OT_Fertilizer"],
    [[6279.34,6835,0],"Jolene's Herbal Farm","OT_Fertilizer","OT_Ganja"],
    [[1352.23,8005,0],"Topolin Berry Farm","OT_Fertilizer","OT_Grapes"],
    [[1223.91,8923.21,0],"Topolin Quarry","","OT_Steel"],
    [[5210.57,5668.39,0],"Huta Lumberyard","OT_Wood","OT_Lumber"],
    [[11000.3,9026.56,0],"Sitnik Plastics","","OT_Plastic"],
    [[11336.9,9461.08,0],"Sitnik Medical","OT_Plastic","FirstAidKit"],
    [[1956.49,6677.26,0],"Campfire Wines","OT_Grapes","OT_Wine"],
    [[11586.7,387.777,0],"Dolnik Lumber Supplies","OT_Wood","OT_Lumber"],
    [[10975.2,1939.36,0],"Livonia Loggers","","OT_Wood"],
    [[4534.43,4715.82,0],"Huta Loggers","","OT_Wood"]
];
OT_factoryPos = [7343.56,2838.22,0];
OT_factoryVehicleSpawn = [7350.82,2841.75,0];
OT_factoryVehicleDir = 67;
OT_connectedRegions = [];
